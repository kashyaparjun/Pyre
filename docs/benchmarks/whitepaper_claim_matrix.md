...
| E5 | empirical | "The Pyre bridge adds approximately 143MB of base memory ... Each agent ... approximately 2.9KB" | benchmark | `memory_scaling.elixir.absolute_base_runtime_bytes`, `bytes_per_agent` | `base in [100,180]MB` and `slope in [2,5]KB/agent` | `base in [80,200]MB` and `slope <= 10KB/agent` | rigorous | Absolute RSS is verdict source; idle-BEAM delta is reported as context. |
