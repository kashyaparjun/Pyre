...
## 4.2 Memory Overhead
The Elixir runtime adds approximately 30MB of base memory (ERTS + OTP libraries). Each agent on the Elixir side consumes 2–5KB (GenServer process + serialized state). The Python side adds no per-agent overhead beyond the handler function reference. A system with 1,000 agents uses approximately 87MB total; 10,000 agents uses approximately 115MB. These figures are well within the resource budgets of typical server deployments and even development laptops.
## 4.3 Throughput
